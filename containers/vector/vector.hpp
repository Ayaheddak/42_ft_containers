	...
	public:
		...
		*/
		...
		/*
			...
			- TODO: diff between NULL and nullptr
		*/
			   ...
			   typename ft::enable_if<!ft::is_integral<InputIterator>::value, InputIterator>::type = InputIterator())
		{
			...
			if (range <= 0)
				return;
			this->_allocator = alloc;
			this->_array = this->_allocator.allocate(range);
			this->_size = range;
		...
		*/
		...
		{
			difference_type at = position - this->_array;
            if (this->_size == this->_capacity)
                this->_capacity == 0 ? reserve(1) : reserve(this->_capacity * 2);
            for (difference_type i = this->_size; i > at; i--)
                this->_allocator.construct(&this->_array[i], this->_array[i - 1]);
            this->_allocator.construct(&this->_array[at], val);
            this->_size++;
            return (this->_array + at);
		}
		...
		void insert(iterator position, size_type n, const value_type &val)
		{
            ...
            for (int i = _size - 1; i >= at; i--)
                this->_allocator.construct(&_array[i + n], _array[i]);
            for (size_type i = 0; i < n; i++, at++)
                this->_allocator.construct(&_array[at], val);
            this->_size += n;
		}
		...
		void insert(iterator position, InputIterator first, InputIterator last, typename ft::enable_if<!ft::is_integral<InputIterator>::value, InputIterator>::type = InputIterator())
		{
            ...
            while (_size + n - i > 0)
            {
                if (_size - i == pos)
                {
                    ...
                    {
                        while (pos--)
                           this->_allocator.construct(&_array[_size - i + pos], *(--last));
                    }
                    catch (...)
                    {
                        for (size_type i = size(); i != 0; i--)
                            this->_allocator.destroy(&_array[i - 1]);
                        _capacity = 0;
                        throw "OOR";
                    }
                    break;
                    ...
                    this->_allocator.construct(&_array[_size - i + n - 1] ,  _array[_size - i - 1]);
                i++;
            }
            _size += n;
        }
